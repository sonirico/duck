...
ok=false
if [ "$stack" = true ] && [ "$api" = true ] && [ "$db" = true ] && [ "$logs_title" = true ] && [ "$volume_tab" = true ]; then
  ok=true
fi

cat > "$receipt_dir/smoke.json" <<EOF
{"ok":$ok,"sha":"$sha","checks":{"stack":$stack,"api":$api,"db":$db,"logs_title":$logs_title,"volume_tab":$volume_tab}}
EOF
