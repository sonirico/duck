...
- **Right panel subtabs**: on a container the right panel cycles through `logs`, `info`, `env`, `top`, `stats`, `vols`, `nets` and `inspect` with `h` / `l` (or `[` / `]`); on a stack it cycles `logs`, `info`, `vols` and `nets`. `enter` on a stack or container jumps to the `info` view; `esc` returns to `logs`. `info` lists services, ports, volumes, networks and mounts (`source -> dest`); `vols` details every mount (type, rw/ro); `nets` shows each network with its ip, gateway and aliases, or the services attached per network on a stack; `stats` shows a live CPU/memory sample that refreshes every 2s while visible.
